...
```{r}
#| eval: false
View(sw_films) #viewing 7 of the star wars movies
```
...
```{r}
sw_films[[4]][["title"]] #index 4, then title
sw_films %>% pluck(4,"title") #goes to 4, then looks for title
```

> The first line of code extracts the "4" and the title separately, while the second code starts with 4 the plucks the title out of the fourth movie on the list
...
#sw_films %>% map("title")
sw_films %>% map_chr("title") #looking for character vector
...
```{r}
sw_films[[1]][["starships"]] #links for starships
```
...
```{r}
sw_films %>% map(., "starships") %>% map_dbl(~length(.)) #length of vectors
map_dbl(sw_films, ~length(.x$starships)) #length of starships 
sw_films %>% map_dbl(., ~length(.x$starships)) #length of starships
```

> The first line of code pulls out "starships", then applies 'length' to each vector to count the URLS.
> The second line of code looks directly for the length of the starships.
> The third line of code is very similar to the second, but it pipes from the 'sw_fims'.
...
> I noticed that the use of hyperdrive type vehicles were primarily used in the first 6 movies which were from Lucasfilms and the 7th movie was from Disney.
...
```{r}
#| label: Star Wars Character List
sw_peeps <- sw_people %>%  {
  tibble(
...
```{r}
#| label: Characters Appear in which films
sw_peeps2 <- sw_peeps %>% 
...
```{r}
#| label: character characteristics
sw_peeps2 %>% 
